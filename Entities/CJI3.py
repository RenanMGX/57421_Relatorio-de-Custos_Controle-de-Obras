...
class CJI3(SAPManipulation):
    def __init__(self, *, date:datetime) -> None:
        ...
        for _file in os.listdir(self.bases_path):
            if _file.lower().endswith((".xlsx", ".xls", ".xlsm")):
                try:
    ...
    def gerar_relatorios_SAP(self, *, objeto:Paths, peps:list=[".po"], gerar_quantos:int=987654321, num_arquivos:int=999999999) -> None:
        contador_gerados = 1
        ...
        objeto.files_extracted_from_sap = {}

        try:
            for centro_custo in objeto.lista_centros:
                for pep in peps:
                    ...
                    for _ in range(5):
                        try:
                            ...
                            #nomes_empreendimentos[centro_custo]
                            empreendimento_for_save:str = f"{centro_custo} - {objeto.nome_empreendimentos.get(centro_custo)} - {datetime.now().strftime('%d-%m-%Y')}.xlsx".upper()
